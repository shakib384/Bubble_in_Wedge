...
/**
The potential() function fills field *pot* with
$$
A_0/(\mathbf{G}\cdot(\mathbf{x} - \mathbf{Z}))^3
$$
with $\mathbf{x}$ the position of the interface defined by $f$.

If *add* is *true*, the potential is added to *pot*. */

void potential (scalar f, scalar pot,
         coord G = {0}, coord Z = {0}, bool add = false)
{

  double A0=4.2875e-13; //6.4e-13; //1.25e-12;  //8e-11;  //1.e-11;  //8e-14;  //3.375e-14;  //1.e-14;  //1.25e-15;
  double h0=1.75e-6;   //0.2e-5; //0.25e-5;   //1.e-5;  //0.5e-5;  //1.e-6;  //7.5e-7;       //5.e-7;  //2.5e-7;
  double n0=3.;
...
#if TREE
  pot.refine = pot.prolongation = curvature_prolongation;
  pot.restriction = curvature_restriction;
#endif
